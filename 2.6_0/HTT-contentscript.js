(function() {
  ...
  function HTTparseResponse(responseText) {
	...
	//
	//alert(tempDiv.getElementById("result_box").textContent);
	//alert(tempDiv.getElementById("destText"));	
	//alert(tempDiv.getElementById("translationLayer"));
	//alert(tempDiv.getElementsByClassName("textinput textlayer textlayer_measurer textlayer_measurer_layer textlayer_measurer_translation")[0].innerText);
	//
	...
	//	
	//var new_result = new Object;
	//new_result['word'] = tempDiv.getElementById("result_box").textContent;
	//results.push(new_result);
	//
    tempDiv = null;
  ...
  function HTTtranslateWord(input) {
    if(input != '') {
	  ...
	  //chrome.extension.sendRequest({'action' : 'xhr', 'url' : 'https://translate.google.com/?hl=ru#en/ru/' + encodeURIComponent(input)}, HTTparseResponse);
	  //chrome.extension.sendRequest({'action' : 'xhr', 'url' : 'https://www.bing.com/translator/?text='+encodeURIComponent(input)+'&from=en&to=ru'}, HTTparseResponse);
	  chrome.extension.sendRequest({'action' : 'xhr', 'url' : 'https://translate.yandex.ua/?ui=ru&lang=en-ru&text='+encodeURIComponent(input)}, HTTparseResponse);	 
    }
  }
